...
const weekTitle = ['San', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
...
const Month = ({ firstMonthDay, daysInMonth, monthIndex, year }) => {
  return (
    <div>
      <ul className="week-title">
        {weekTitle.map(item => {
          return (
            <li className="day-name" key={item}>
              {item}
            </li>
          );
        })}
      ...
      <div className="week-line">
        {getMonthDays(firstMonthDay, daysInMonth).map(item => (
          <Week year={year} monthIndex={monthIndex} week={item} />
        ))}
      </div>
    </div>
  );
